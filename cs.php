<?php
session_start();
echo($_SESSION['username']);
				$message=$_SESSION['username'];
				//echo "<script type='text/javascript'>alert(' $message (Student)');</script>";
?>
<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" type="text/css" href="cataloguesystem.css">
</head>
<body>
	<div class="wrapper">
<div class="topnav">

		    <pre class="tab2"></pre><button class="button3"><a href="login.html">login</a></button>
		    <pre class="tab"></pre>
		    <button class="button4"><a href="signup.php">signup</a></button>
		    

		    <pre class="tab4"></pre><a href=https://www.facebook.com/><img class="image1" src="static\fb.png" ></a>

		    <pre class="tab"></pre><a href=https://www.instagram.com/><img class="image2" src="static\instagram.png" ></a>

		    <pre class="tab"></pre><a href=https://www.youtube.com/?gl=IN><img class="image3" src="static\youtube.png" ></a>

		    <pre class="tab6"></pre>

		</div>
		<div class="middlenav">
			<div class="midnav_short">
				<ul style="float:right">		
				<a href="homepage.html">Home</a> &emsp;
				<a href="aboutpage.html">About</a> &emsp;
				<a href="Blog.html">Forum</a>
			</ul>
			</div>
		</div>
		<div class="maindiv1">
		  	<div class="gridee_cslist">
		  		<div class="one11">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5321 - Software Testing</center></h4>
					</div>
		  		</div>
		  		<div class="two22">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>		
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5334 - Data Mining<center></h4>
					</div>
		  		</div>
		  		<div class="three33">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5364 - Robotics</center></h4>
					</div>
		  		</div>
		  		<div class="four44">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5346-Networks</center></h4>
					</div>
		  		</div>
		  		<div class="five55">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5370 - Bioinformatics</center></h4>
					</div>
				</div>
				<div class="six66">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5330-Database Systems</center></h4>
					</div>
				</div>
				<div class="seven77">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5321 - Software Testing</center></h4>
					</div>
				</div>
				<div class="eight88">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 6367-Computer vision</center></h4>
					</div>
				</div>
				<div class="nine99">
		  			<div class="courseimage2">
			  				<img src="static/empty.png"></>				
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5321 - Software Testing</center></h4>
					</div>
		  		</div>
		  		<div class="ten1010">
		  			<div class="courseimage2">
		  				<img src="static/empty.png"></>					
					</div>
					<div class="coursename2">
					<br>
					<h4><center>CSE 5382-Secure Programming</center></h4>
					</div>

		  		</div>
		  		</div>
		</div>
		</div>
<footer>
	<div class="Foot">
		<br>
		<br>
		<div class="login_txt">
		<hr style="width:90%;margin-left:05%; align-items: center;">
		<br>
			<font color="#FFFFFF" style="margin-left: 05%;">Created by</font>
			<div class="sub_txt">
			<font color="#FFFFFF" style="margin-left: 75%;">All Rights Reserved</font>
			</div>
	</div>
	</div>
</footer>
	</div>
</body>
</html>
